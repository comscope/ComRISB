...
import numpy as np
from pygrisb.iface.wannierio import get_wannier_data
...
class tb_model(object):
    ...
    def k_uniform_mesh(self, mesh_size):
        ...
        if self._dim_k == 1:
            ...
            k_vec = k_vec/norm
            # final reshape
            k_vec = k_vec.transpose([1, 2, 0]).reshape(
                [use_mesh[0]*use_mesh[1], 2])
        elif self._dim_k == 3:
            # get a mesh
        ...
        if report == True:
            if self._dim_k == 1:
                ...
                for n in range(1, n_nodes):
                    ...
                    print('  length = '+dk_str+'  from ',
                          k_list[n-1], ' to ', k_list[n])
