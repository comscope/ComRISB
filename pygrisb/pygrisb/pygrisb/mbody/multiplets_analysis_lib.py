...
def get_rho_histogram(rho, S=None, L=None, J=None, num_ev=0, Rpr_list=None):
    ...
    from pygrisb.mbody.local_operator_factory import get_label_list
    print(" Get labels.")
    ...
    from pygrisb.math.matrix_util import eigen_space
    es = eigen_space(vals)
    vals = es.get_reduce_labels(vals, method="sum")
    s_label = es.get_reduce_labels(s_label, method="average")
    l_label = es.get_reduce_labels(l_label, method="average")
    j_label = es.get_reduce_labels(j_label, method="average")
    idx = es.idx
    ...
    if Rpr_list is not None:
        # check commutation
        from pygrisb.symm.atom_symm import check_commute_G
        check_commute_G(rho, Rpr_list)

        from pygrisb.symm.atom_symm import get_characters_espace, \
                check_sum_chi2_1
        chi_label, _ = get_characters_espace(Rpr_list, vecs)
        check = check_sum_chi2_1(chi_label)
        if check != "OK":
            warnings.warn("chi-sum error!")
    else:
        chi_label = None
...
def get_local_histogram(imp, op_list=["S2","L2","J2"], num_ev=0,
    ...
    with h5py.File('HEmbed.h5'.format(imp), 'r') as f:
        _nbot = f[f'/impurity_{imp}'].attrs['nval_bot']
        _ntop = f[f'/impurity_{imp}'].attrs['nval_top']
    if n_bot is None:
        n_bot = _nbot
    if n_top is None:
        n_top = _ntop
    ...
    from pygrisb.mbody.local_operator_factory import get_local_operators
    for ival in range(n_bot, n_top+1):
        print(" Valence = {}".format(ival))
        # Get reduced density matrix.
        with h5py.File(f'HEmbedAnalysis_{imp}.h5', 'r') as f:
