...
import numpy as np
import h5py, argparse, os, sys
from pygrisb.mbody.multiplets_analysis_lib import get_local_histogram
...
def get_inline_args():
    ...
    parser.add_argument("-i", "--imp", type=int, default=0,
            help="inmurity index (int: 0)")
    parser.add_argument("-e", "--nev", type=int, default=50,
            help="number of eigen-vectors to be solved (int: 50)")
    parser.add_argument("-l", "--nlabel", type=int, default=5,
            help="number of states to be labelled (int: 5)")
...
def driver():
    ...
    if not os.path.isfile(f'HEmbedAnalysis_{args.imp}.h5'):
        print("please run the multiplet analysis code first!")
        sys.exit()

    calc_multiplets = True
    if os.path.isfile("multiplets.h5"):
        with h5py.File("multiplets.h5", "r") as f:
            calc_multiplets = not (f"/impurity_{args.imp}" in f)

    if calc_multiplets:
        print("calculate multiplets")
        calc_save_atomic_states(imp=args.imp, num_ev=args.nev)

    plot_atomic_states(imp=args.imp, num_label=args.nlabel)
