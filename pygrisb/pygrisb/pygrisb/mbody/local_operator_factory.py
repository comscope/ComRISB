...
from pygrisb.io.h5io import get_coo_matrix
...
def get_linear_operators(nij, uij):
    ...
    if nij is None:
        return None
    res = None
    for i, n1, u1 in zip(it.count(), nij, uij):
        for j, n12, u12 in zip(it.count(), n1, u1):
            if n12 is None:
                continue
            if abs(u12)>1.e-12:
                if res is None:
...
def get_local_operators(imp, ival, op_list):
    ...
    with h5py.File('GParam.h5', 'r') as f:
        svec = []
        svec.append(f['/impurity_'+str(imp)+'/sx'][()])
        svec.append(f['/impurity_'+str(imp)+'/sy'][()])
        svec.append(f['/impurity_'+str(imp)+'/sz'][()])
        lvec = []
        lvec.append(f['/impurity_'+str(imp)+'/lx'][()])
        lvec.append(f['/impurity_'+str(imp)+'/ly'][()])
        lvec.append(f['/impurity_'+str(imp)+'/lz'][()])
...
def get_nij_op(l='d', ival='1'):
    ...
    fname = pkg_resources.resource_filename('pygrisb', './mbody/mbody.h5')
...
def get_lrot_op(lvec, lie_jeven_params, l='d', ival='1'):
    ...
    from pygrisb.symm.atom_symm import get_representation
