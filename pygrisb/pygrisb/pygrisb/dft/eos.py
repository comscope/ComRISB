...
def h5get_mfit_ev(nmesh_fac=10, fsave='results.h5', path='/lapw'):
    ...
    with h5py.File(fsave, 'r') as f:
        e_list = f[path+'/e_list'][()]
        v_list = f[path+'/v_list'][()]
    ...
    ph = Murnaghan_pv(murnpars, vh)*units.ev_per_cube_angstrom_to_giga_pasca
...
def eos_spline(v, e, tol):
    ...
    ph = [-s.derivatives(i)[1] * \
            units.ev_per_cube_angstrom_to_giga_pasca for i in vh]
    bh = [s.derivatives(i)[2] * vh[i] * \
            units.ev_per_cube_angstrom_to_giga_pasca for i in vh]
    return vh, eh, ph, bh
