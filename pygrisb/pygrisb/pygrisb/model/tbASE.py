...
class TB(object):
    ...
    def eigens(self, ikp=0, kpt=numpy.zeros((3)), hk=None):
        """
    ...
    def get_dos(self,  kps_size=None, saveto=None, dos_mesh=None, eta=1e-2):
        ...
        eks = numpy.array([self.eigens(i, kpt)[0]
    ...
    def save_bareham(self, kpts):
        numk = len(kpts)
        norbs = self.Atoms.nspinorbitals
        ek_list = numpy.zeros((numk, norbs), dtype=numpy.float)
        vk_list = numpy.zeros((numk, norbs, norbs), dtype=numpy.complex)
        hk_list = numpy.zeros((numk, 1, norbs, norbs), dtype=numpy.complex)
        for ik, kpt in enumerate(kpts):
            hk, _ = self.Hk(ikp=ik, kpt=kpt)
            ek, vk = self.eigens(hk=hk)
            hk_list[ik, 0, :, :] = hk
            ek_list[ik, :] = ek
            vk_list[ik, :, :] = vk
        with h5py.File('GBareH.h5', 'a') as f:
            f["/ispin_0/e_list"] = ek_list
            f["/"].attrs["mode_hk"] = 0
            f["/ispin_0/U_PSIK0_TO_HK0_BASIS_LIST"] = vk_list.swapaxes(1, 2)
            f["/ispin_0/HK0_LIST"] = hk_list.swapaxes(2, 3)
