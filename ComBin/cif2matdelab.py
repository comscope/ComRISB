...
def establish_method(ini_struct):
    """
...
def establish_mt_radii(ini_struct, input_element_rad=None):
    """
    ...
    if input_element_rad is not None:
        element_rad.update(input_element_rad)
    print(f"initial element_rad: {element_rad}")
    #
    ...
    print(f"final element_rad: {element_rad}")
    return ini_struct

def check_rkmax(ini_struct):
    # minimal mt radius
    rmt_min = min(ini_struct["element_rad"].values())
    # Kmax
    kmax = ini_struct["Kmax"]
    print(f"Smallest R_MT = {rmt_min:.2f}, Kmax= {kmax:.2f}")
    print(f"R_MT * Kmax = {rmt_min*kmax:.2f}.")
...
def establish_Kmax(ini_struct,Kmax=0):
    ...
    print(f"Kmin = {Kmin:.2f}, Kmax_init = {Kmax:.2f}, Klim = {Klim:.2f}")
    Kfin = max(Kmax,Kmin,1)
    if ini_struct["method"] != "dft":
        Kfin = min(Kfin,Klim)
...
def write_inifile(ini_struct,inifile):
    ...
    inifile.write("K_POINT  ndiv= 10 10 10  metal=T n_k_div= 19 k_line=010\n")
