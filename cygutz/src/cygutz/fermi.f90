subroutine eweigh(ef,weight,nemax,k,jspin,ne,eb,nbmax,cordeg,io)
        ...
        do jspin1=1,jspin
           ...
           do while(nn.le.nnn)
              ...
              if(ifcp.eq.1) then
                 if(ifcpt.eq.1) then
                    if(io>0)then
                        write(io,'("k-pnt spin band  energy          &
                                &old/new occ.    ")')
                    endif
                    ifcpt=0
                 endif
                 do icp=0,ndeg-1
                    if(io>0)then
                        write(io,50)kk,jspin1,nn,eb(nn+icp,kk,jspin1), &
                                &weight((nn+icp)+(jspin1-1)*nnn,kk)
                    endif
                    weight((nn+icp)+(jspin1-1)*nnn,kk) = wecp/dble(ndeg)
                    if(io>0)then
                        write(io,'(f9.6)')weight((nn+icp)+(jspin1-1)*nnn,kk)
                    endif
                 enddo
              endif
              nn=nn+ndeg
...
!
   ...
   8 continue
    if(io>0)then
        write(io,*) '  number of occupied bands:',nbmax
        write(io,*) '  highest energy:',emax
    endif
     return
end subroutine eweigh
!
subroutine dos(nb,nkp,eb,wght,rntot,ef,w,nwx,icor,io)       
  !use parallel
  !     **                                                              *
  ...
  call efermi(rntot,ef,tolmax,nb,nkp,eb,w,nwx,io)                     
!
    if(io>0)then
        write(io,*) '  fermi energy at ',ef
    endif
  !     -----------------------------------------------------------------
  !     --  calculate weights                                           -
  !     -----------------------------------------------------------------
  call samfac(nb,nkp,eb,ef,wght,w,nwx,icor)                             
!
...
!
    if(io>0)then
        if(dabs(sum-rntot).gt.tolmax) then 
           write(io,9000) sum,rntot
        else                                                             
           write(io,*) '  sum rule of tetrahedron integration checked '
        end if
    endif
  return                                                           
...
!
!     .....................................................efermi......
subroutine efermi(rntot,ef,tolmax,nb,nkp,eb,w,nwx,io)               
  !     **                                                              *
...
!     -----------------------------------------------------------------
      if(tol.gt.tolmax) then                                           
        ...
        if(drval(w(onos),ip)>rntot) then
          ...
          enddo
          if(io>0)then
            write(io,'(" warning: ip = ", i0, " sgift back to jp = ", &
                    &i0)') ip, jp
          endif
        ...
        if(drval(w(onos),ip+1)<rntot) then
          ...
          enddo
          if(io>0)then
              write(io,'(" warning: ip = ", i0, " sgift forward to jp = ",&
                    & i0)') ip, jp
          endif
...
!
        if(estep.lt.1.d-10) then
            if(io>0)then
                write(io,*) 'warning: ef not accurate, new emin,emax,&
...
!
!     .....................................................samfac......
      subroutine samfac(nb,nkp,eb,ef,wght,w,nwx,icor)                       
!     **                                                              *
